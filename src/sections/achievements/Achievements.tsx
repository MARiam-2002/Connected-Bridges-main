...
const Counter = memo(({ value, isInView }: { value: string; isInView: boolean }) => {
    ...
    return (
        <span>
            {parts.map((part, index) => (
                /\d+/.test(part)
                    ? <motion.span key={index}>{rounded}</motion.span>
...
export default function Achievements() {
    ...
    const [containerRef, isInView] = useInView({ threshold: 0.2, rootMargin: '50px', triggerOnce: true });

    const achievementsData = [
        {
            icon: <FaAward size={40} className="text-[var(--dark-blue-color)]" aria-hidden="true" />,
            value: i18n.language === "ar" ? "15 +" : "+ 15",
            title: t("overview.achievements.cards.years.title"),
        },
        {
            icon: <GoProject size={40} className="text-[var(--dark-blue-color)]" aria-hidden="true" />,
            value: i18n.language === "ar" ? "100 +" : "+ 100",
            title: t("overview.achievements.cards.projects.title"),
        },
        {
            icon: <FaRegBuilding size={40} className="text-[var(--dark-blue-color)]" aria-hidden="true" />,
    ...
    return (
        <React.Fragment>
            <motion.div
                ...
                `}
                role="region"
                aria-label={t("overview.achievements.title") ?? "Achievements"}
            >
                {achievementsData.map((achievement, index) => (
                    ...
                    >
                        ...
                        <motion.h3 variants={achievementsTitle} className="text-xl font-semibold text-shadow-md">
                            {achievement.title}
                        </motion.h3>
                    </div>
                ))}
